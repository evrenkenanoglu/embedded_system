...
class TestMatterHIL:
    ...
    # def test_00_connect_wifi_via_serial(self):
    #     """
    #     Connects to the ESP32 via USB Serial.
    #     - Waits for console (parameterized log message)
    #     - Factory resets
    ...
    #     try:
    #         # Open the serial port with a short timeout for non-blocking reads
    #         ser = serial.Serial(Config.SERIAL_PORT, Config.SERIAL_BAUD, timeout=0.1)
            
    #         def wait_for_log(pattern, timeout_sec, send_nl=False):
    #             """
    #             Helper to read serial logs until a regex pattern is found.
    #             Prints the logs to the console for CI/CD debugging.
    #             """
    #             end_time = time.time() + timeout_sec
    #             while time.time() < end_time:
    #                 # Periodically send a newline to provoke a CLI prompt if idle
    #                 if send_nl and int(time.time()) % 2 == 0:
    #                     ser.write(b'\r\n')
                        
    #                 line = ser.readline().decode('utf-8', errors='ignore').strip()
    #                 if line:
    #                     print(f"[ESP32] {line}") # Print device logs to pytest console
    ...
    #         # We look for the parameterized INIT pattern OR the CLI prompt ">" in case it booted 10 mins ago
    #         ready_pattern = f"({CONSOLE_INIT_PATTERN}|>)"
    #         ready = wait_for_log(ready_pattern, timeout_sec=10, send_nl=True)
    #         assert ready, "Timed out waiting for Matter console to initialize."
    ...
    #         print("\n[STEP 3] Waiting for ESP32 to reboot and initialize Matter...")
    #         # Strictly wait for the boot log this time (no newline spam needed)
    #         reboot_ready = wait_for_log(CONSOLE_INIT_PATTERN, timeout_sec=25, send_nl=False)
    #         assert reboot_ready, f"Timed out waiting for ESP32 to reboot and print: {CONSOLE_INIT_PATTERN}"
            
    #         # Brief pause to ensure background Matter tasks are fully spawned after the init print
    #         time.sleep(1)
    ...
    #         # 5. Wait until WIFI_EVENT_STA_CONNECTED / got ip
    #         print("\n[STEP 5] Waiting for Wi-Fi connection and IP Address...")
    #         # Matches strings like "got ip: 192.168.31.71" or "sta ip: 192.168.31.71"
    #         ip_match = wait_for_log(r"(?:got ip:|ip:\s*)(\d+\.\d+\.\d+\.\d+)", timeout_sec=20)
    #         assert ip_match, "Timed out waiting for Wi-Fi connection or IP address!"

    #         # 6. Save device IP locally
    ...
    #         pytest.fail(f"Could not open serial port {Config.SERIAL_PORT}. Is 'idf.py monitor' still running? Error: {e}")
