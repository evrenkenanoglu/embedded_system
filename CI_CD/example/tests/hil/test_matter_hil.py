...
class TestMatterHIL:
    ...
    def test_00_connect_wifi_via_serial(self, dut: pytest_embedded.Dut):
        ...
        try:
            ...
            match = dut.expect(
                re.compile(r"(?:got ip:|ip:\s*)(\d+\.\d+\.\d+\.\d+)"), timeout=30
            )
            ...
            pytest.fail(
                f"Serial communication failed. Is 'idf.py monitor' still running? Error: {e}"
            )
    ...
    def test_01_commission_device(self):
        ...
        for attempt in range(3):
            ...
            success = re.search(
                r"Device commissioning completed with success", output, re.IGNORECASE
            )
            if success:
    ...
    def test_02_verify_connectivity(self):
        ...
        assert (
            "Espressif" in output or "VendorName" in output
        ), f"Failed to read Basic Information cluster! Output: {output}"
    ...
    def test_03_find_endpoints(self):
        ...
        assert (
            len(endpoint_matches) == num_entries
        ), f"Expected {num_entries} endpoints but found {len(endpoint_matches)}!"
    ...
    def test_04_read_on_all_endpoints(self):
        ...
        for ep in ENDPOINTS:
            ...
            assert (
                onoff_match
            ), f"Failed to read OnOff cluster on endpoint {ep}! Output: {output}"
            print(
    ...
    def test_05_set_on_all_endpoints(self):
        ...
        for ep in ENDPOINTS:
            ...
            assert (
                success
            ), f"Failed to write OnOff cluster on endpoint {ep}! Output: {output}"
