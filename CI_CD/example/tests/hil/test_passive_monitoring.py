...
def test_physical_button_and_switch_events(dut: Dut):
    ...
    dut.expect(
        r"\[DEBUG\] Executing switch command: Switch Index=4, Target State=1", timeout=5
    )
    dut.expect(r"\[DEBUG\] Setting switch 4 to state 1", timeout=5)
    dut.expect(
        r"\[DEBUG\] Notifying HTTP switch state change: Switch Index=4, New State=1",
        timeout=5,
    )
    ...
    dut.expect(r"\[DEBUG\] Received button event: Index=5, Event=0", timeout=60)
    dut.expect(
        r"\[DEBUG\] Executing switch command: Switch Index=4, Target State=0", timeout=5
    )
