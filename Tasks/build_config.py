...
def _load_ssot_config(workspace_root: Path) -> Dict[str, Any]:
    ...
    if not ssot_path.exists():
        raise FileNotFoundError(
            f"Master project configuration not found at: {ssot_path}"
        )
...
@task(
    name="all",
    default=True,
    help={"dry_run": "Print target paths without writing files"},
)
def generate(c: Context, dry_run: bool = False) -> None:
    ...
    if dry_run:
        print(
            f"[DRY-RUN] Would generate all build configuration artifacts in: {workspace}"
        )
        return
