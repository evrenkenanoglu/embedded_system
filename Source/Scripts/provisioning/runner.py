...
USAGE EXAMPLES (Run from workspace root):
    1. Interactive Selection Menu (Zero arguments):
        python embedded_system/Source/Scripts/provisioning/runner.py

    2. Test entire pipeline in dry-run mode:
        python embedded_system/Source/Scripts/provisioning/runner.py --all --dry-run

    3. Generate encrypted NVS only:
        python embedded_system/Source/Scripts/provisioning/runner.py --nvs

    4. Sign compiled firmware and update server manifest:
        python embedded_system/Source/Scripts/provisioning/runner.py --sign

    5. Flash physical silicon:
        python embedded_system/Source/Scripts/provisioning/runner.py --provision
"""
...
def run_main(step: str, dry_run: bool = False) -> int:
    """Executes main.py with resolved configuration and step arguments."""
    cmd = [sys.executable, str(MAIN_PY), "--config", str(CONFIG_YAML), "--step", step]
    if dry_run:
        cmd.append("--dry-run")
...
def interactive_menu() -> None:
    """Displays an interactive selection menu when no CLI arguments are supplied."""
    while True:
        print("\n" + "=" * 50)
        ...
        if choice == "1":
            run_main("all", dry_run=True)
            break
        elif choice == "2":
            run_main("all", dry_run=False)
            break
        elif choice == "3":
            run_main("nvs", dry_run=False)
            break
        elif choice == "4":
            run_main("sign", dry_run=False)
            break
        elif choice == "5":
            run_main("provision", dry_run=True)
            break
        elif choice == "6":
            run_main("provision", dry_run=False)
            break
        elif choice == "0":
            print("\nExiting.")
            sys.exit(0)
...
def main() -> int:
    ...
    if args.all:
        return run_main("all", dry_run=args.dry_run)
    elif args.nvs:
        return run_main("nvs", dry_run=args.dry_run)
    elif args.sign:
        return run_main("sign", dry_run=args.dry_run)
    elif args.provision:
        return run_main("provision", dry_run=args.dry_run)
