...
class Analyzer:
    def __init__(self):
        self.script_dir = Path(
            __file__
        ).parent.parent  # Go up to static_code_analysis dir
    ...
    def get_active_standards(self, standard_list):
        ...
        print(
            f"🔍 Found {len(active)} active standards out of {len(standard_list)} total"
        )
        return active
    ...
    def run_analysis(self, target_file, standard_list, output_format="json"):
        ...
        for standard in active_standards:
            if standard.is_available():
                ...
                print(
                    f"⚠️  Rule file not found for {standard.name}: {standard.rule_file}"
                )
        ...
        success, result = self.output_handler.run_by_format(
            cmd, active_standards, target_file, output_format
        )
    ...
    def _run_simple_output(self, cmd, active_standards, target_file):
        ...
        success, result = self.output_handler.run_simple_output(
            cmd, active_standards, target_file
        )
        return success

    def _run_json_output(self, cmd, active_standards, target_file):
        """Legacy method - redirects to output handler"""
        success, result = self.output_handler.run_json_output(
            cmd, active_standards, target_file
        )
        return success
