...
class Semgrep_Output_Handler:
    ...
    def _generate_report_filename(
        self, format_type, target_file=None, custom_prefix=None
    ):
    ...
    def run_simple_output(self, cmd, active_standards, target_file):
        ...
        try:
            ...
            if result.stdout:
                ...
                simple_report_file = self._generate_report_filename(
                    "simple", target_file
                )
        ...
        try:
            ...
            for standard in active_standards:
                count = sum(
                    1
                    for r in results
                    if standard.name.lower().replace(" ", "_")
                    in r.get("check_id", "").lower()
                )
