...
#define BAD_MACRO(x, y) \
    x = 10;             \
    y = 20;             \
    printf("Done") // Should use do-while(0)

// Good macro example for comparison
#define GOOD_MACRO(x, y) \
    do                   \
    {                    \
        x = 10;          \
        y = 20;          \
...
int duplicate_function(int a, int b)
{ // Second definition - VIOLATION
    return a * b;
}

// MISRA C++ 15-3-4 violation: Exception specification (deprecated)
void old_exception_spec() throw(std::bad_alloc)
{ // VIOLATION
    throw std::bad_alloc();
}
...
void magic_numbers_example()
{
    ...
    if (array_size > 50)
    { // VIOLATION: Magic number 50
        printf("Large array\n");
    }

    for (int i = 0; i < 25; i++)
    { // VIOLATION: Magic number 25
        printf("Index: %d\n", i);
    }
