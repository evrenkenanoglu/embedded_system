...
HttpUriWebsocket::HttpUriWebsocket(const char* uriName, esp_err_t (*handler)(httpd_req_t*), void* user_ctx, const char* htmlContent)
    : HttpUri(uriName, HTTP_GET, handler ? handler : default_app_interface_get_handler, user_ctx, true)
...
static error_t default_app_interface_get_handler(httpd_req_t* req)
{
    HttpUriWebsocket* htmlPage = static_cast<HttpUriWebsocket*>(req->user_ctx);
