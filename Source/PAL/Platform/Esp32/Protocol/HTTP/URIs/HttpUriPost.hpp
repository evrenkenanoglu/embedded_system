...
public:
    ...
    HttpUriPost(const char* uriName, Handler handler = nullptr, void* user_ctx = nullptr) noexcept;

    virtual ~HttpUriPost() = default;

private:
    // Default portable POST handler: if body present echoes it, otherwise returns 204.
    static int default_post_handler(const char* req_ptr, std::size_t req_len, char* resp_buf, std::size_t resp_buf_len, void* user_ctx) noexcept;
};