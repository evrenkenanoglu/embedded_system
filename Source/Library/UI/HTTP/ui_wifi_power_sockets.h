...
\
                        for (let i = 0; i < 10; i++) {\
                            socketStateMap.set(i, 'off');\
                        }\
                        socketStateMap.set(ButtonIndexAll, 'off');\
\
                        async function handleSwitchState(button, index) {\
                            const isOn = socketStateMap.get(index) === 'on';\
                            const newState = isOn ? 'off' : 'on';\
                            const newStateBool = isOn ? 0 : 1;\
\
\
...
\
                            try {\
                                const response = await fetch('/power-switches-control', {\
                                    ...
                                    body: JSON.stringify({ socketId: index, state: newStateBool }),\
                                ...
                                if (response.ok) {\
                                    if (index === ButtonIndexAll) {\
                                        sockets.forEach((button, i) => {\
                                            ...
                                            button.textContent = `Socket ${i + 1} ${newState.toUpperCase()}`;\
...
\
                                        button.classList.toggle('on', !isOn);\
                                        button.classList.toggle('off', isOn);\
                                        button.textContent = `Socket ${index + 1} ${newState.toUpperCase()}`;\
                                    }\
